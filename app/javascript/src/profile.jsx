...
const Profile = () => {
  ...
  const refreshProfile = () => {
    fetch('/api/user/reviews', safeCredentials({
      method: 'GET',
    }))
      .then(handleErrors)
      .then(data => {
  ...
  const handleDelete = (id) => {
    fetch(`/api/reviews/${id}`, safeCredentials ({
      method: 'DELETE',
      body: JSON.stringify({
              ...
              <div className="row">
                ...
                {reviews.map(review => (
                  <div className="row mb-4 w-100 " key={review.id}>
                    <div className=" col-12 p-4 bg-white rounded shadow-sm position-relative">
                    <button className="btn delete position-absolute top-0 end-0 m-1 text-danger" onClick={()=>handleDelete(review.id)} >Delete</button>
